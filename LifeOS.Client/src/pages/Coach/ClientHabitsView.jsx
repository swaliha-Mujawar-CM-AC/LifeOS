...
export function ClientHabitsView({ coach }) {
  ...
  useEffect(() => {
    if (!selectedUser) { setHabits([]); return; }
    setLoadingHabits(true);
    fetch(`${API}/Habit/user/${selectedUser.id}`)
      .then(r => r.ok ? r.json() : [])
      .then(setHabits)
      .catch(() => setHabits([]))
  ...
  // Filter habits
  const filteredHabits = habits.filter(h => categoryFilter === 'All' || (h.category || 'General') === categoryFilter);

  // Statistics
  const totalHabits = filteredHabits.length;
  const completedToday = filteredHabits.filter(h => h.isCompletedToday).length;
  const completionRate = totalHabits > 0 ? Math.round((completedToday / totalHabits) * 100) : 0;

  // Streak data
  const averageStreak = totalHabits > 0 ? Math.round(filteredHabits.reduce((sum, h) => sum + h.streak, 0) / totalHabits) : 0;
  const maxStreak = totalHabits > 0 ? Math.max(...filteredHabits.map(h => h.streak)) : 0;
  ...
  return (
    <div style={{ padding: 0, overflowY: 'auto', height: 'calc(100vh - 130px)' }}>
      <div className="page-header">
        <div className="page-title">📊 Client Habits Analytics</div>
      </div>
      ...
      {selectedUser && (
        <>
          {/* Filter Bar */}
          <div className="dashboard-card" style={{ padding: '1.15rem', marginBottom: '1rem', display: 'flex', gap: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'var(--grid-coach-stats-cols, repeat(4, 1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
            <div className="dashboard-card" style={{ padding: '1rem', textAlign: 'center', borderTop: '3px solid var(--accent-blue)', marginBottom: 0 }}>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Active Habits</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--text-primary)', marginTop: '0.25rem' }}>{totalHabits}</div>
            </div>
            <div className="dashboard-card" style={{ padding: '1rem', textAlign: 'center', borderTop: '3px solid var(--accent-green)', marginBottom: 0 }}>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Done Today</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--accent-green)', marginTop: '0.25rem' }}>{completedToday}</div>
            </div>
            <div className="dashboard-card" style={{ padding: '1rem', textAlign: 'center', borderTop: '3px solid var(--alert-orange)', marginBottom: 0 }}>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Avg Streak</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--alert-orange)', marginTop: '0.25rem' }}>{averageStreak} days</div>
            </div>
            <div className="dashboard-card" style={{ padding: '1rem', textAlign: 'center', borderTop: '3px solid var(--accent-purple)', marginBottom: 0 }}>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Max Streak</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--accent-purple)', marginTop: '0.25rem' }}>{maxStreak} days</div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'var(--grid-coach-charts-cols, 1.2fr 1fr)', gap: '1.25rem' }}>
            {/* Category Habits rate */}
            <div className="dashboard-card" style={{ padding: '1.25rem', marginBottom: 0 }}>
              <h3 style={{ fontSize: '0.92rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--text-primary)' }}>
                Category Habit Progress (Today)
              </h3>
              {loadingHabits ? (
                <p style={{ color: 'var(--text-secondary)' }}>Loading…</p>
              ) : Object.keys(categoryBreakdown).length === 0 ? (
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>No habits found matching filter.</p>
              ) : (
            ...
            <div className="dashboard-card" style={{ padding: '1.25rem', marginBottom: 0, display: 'flex', flexDirection: 'column', justify: 'center', alignItems: 'center' }}>
              ...
              <div style={{ width: '120px', height: '120px', borderRadius: '50%', border: '8px solid var(--border-color)', borderTopColor: 'var(--accent-green)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', margin: '1rem 0' }}>
                ...
                <span style={{ fontSize: '0.62rem', color: 'var(--text-secondary)', fontWeight: 700, textTransform: 'uppercase' }}>Consistency</span>
              </div>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textAlign: 'center', fontWeight: 600, padding: '0 0.5rem' }}>
                Shows consistency stats of habits completed by {selectedUser.username} today. Maintain high streaks to build compound discipline.
              </p>
            </div>
          </div>
          ...
          <div className="dashboard-card" style={{ padding: '1.25rem', marginTop: '1rem' }}>
            <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.75rem' }}>
              💬 Send Habit Advice to {selectedUser.username}
            </h3>
            <textarea value={feedback} onChange={e => setFeedback(e.target.value)}
              placeholder="Write advice based on habit completion streaks and progress..." rows={3} className="form-textarea" />
